...
$PaddleYInterval        =    (2*$APAphys_y+$APAGap_y-$ArapucaOut_y-2*$APAFrameZSide_y)
...
EOF
        ...
        # Make the workspace have only one center APA with CPAs and the full drift on either side
            ...
            place_OpDets(($APACenter_x_test_mod_1), $APACenter_y, $APACenter_z, $apa_i);
            place_OpDets(($CPA_0_x+2), $APACenter_y, $APACenter_z, $apa_i);  
